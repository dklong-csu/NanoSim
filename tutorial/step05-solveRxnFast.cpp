...
std::vector<double> simulateRxnsSparse(const Eigen::Vector<realtype, Eigen::Dynamic> & parameters){
  ...
  for (unsigned int i=0; i<n_solves;++i){
    ...
    if (i == 0){
      // output solution
      std::ofstream outputfilePSD;
      std::ofstream outputfileSize;
      std::string filenamePSD = "step05-sparsesolve-" + std::to_string(max_size) + "-particles-PSD.txt";
      std::string filenameSize = "step05-sparsesolve-" + std::to_string(max_size) + "-particles-sizes.txt";
      outputfilePSD.open(filenamePSD.c_str());
      outputfileSize.open(filenameSize.c_str());
      // just PSD -- starts at index 3
      for (sunindextype size=3;size<=max_size;++size){
        ...
        const realtype conc = lin_alg.vectorGetValue(sol, idx);
        const realtype sizeL = 0.3 * std::pow(size - 0.5, 1./3.);
        const realtype sizeC = 0.3 * std::pow(size * 1.0, 1./3.);
        const realtype sizeR = 0.3 * std::pow(size + 0.5, 1./3.); 
        outputfilePSD << conc / (sizeR - sizeL) << "\n";
        outputfileSize << sizeC << "\n";
      }
      outputfilePSD.close();
  ...
  */
  ...
  for (unsigned int i=0; i<n_solves;++i){
    ...
    if (i == 0){
      ...
      std::ofstream outputfilePSD;
      std::ofstream outputfileSize;
      std::string filenamePSD = "step05-densesolve-eMoM-" + std::to_string(max_size_filename) + "-particles-PSD.txt";
      std::string filenameSize = "step05-densesolve-eMoM-" + std::to_string(max_size_filename) + "-particles-sizes.txt";
      outputfilePSD.open(filenamePSD.c_str());
      outputfileSize.open(filenameSize.c_str());
      /*
      ...
      */
      for (sunindextype size=3;size<=max_size;++size){
        ...
        auto idx = my_rxns.species_to_index_map.find("__PARTICLE__" + std::to_string(size))->second;
        const auto conc = lin_alg.vectorGetValue(sol, idx); 
        outputfilePSD << conc / (sizeR - sizeL) << "\n";
      }
      /*
